...
class HostReservationsOverTimeChart extends ChartWidget
{
    ...
    {
        return __('panel.host.reservations_over_time');
    }
    ...
    protected function getData(): array
    {
        ...
        $propertyIds = Property::where('host_id', Auth::guard('provider')->id())->pluck('id')->toArray();
        ...
        $reservations = PropertyReservation::query()
            ...
            ->get()
            ->groupBy(fn($reservation) => Carbon::parse($reservation->check_in)->format('Y-m-d'));
        ...
        return [
            'datasets' => [
                [
                    'label' => __('panel.host.reservations'),
                    'data' => $data,
                    'fill' => false,
