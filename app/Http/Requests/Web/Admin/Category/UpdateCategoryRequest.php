...
class UpdateCategoryRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $parentId = $this->input('parent_id'); // Get parent_id from input
        ...
        return [
            'name_en' => [
                ...
                'string',
                'min:3',
                Rule::unique('categories', 'name->en')->ignore($categoryId) // Ignore current category for update
            ],
            'name_ar' => [
                'required',
                'string',
                'min:3',
                Rule::unique('categories', 'name->ar')->ignore($categoryId) // Ignore current category for update
            ],
            'priority' => [
                'required',
                new UniquePriorityWithinParent($parentId, $categoryId) // Ensure unique within parent
            ],
            'image' => [
                'nullable',
                'image',
