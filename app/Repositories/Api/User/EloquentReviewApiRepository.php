...
class EloquentReviewApiRepository implements ReviewApiRepositoryInterface
{
    ...
    public function addReview($data)
    {
        ...
        try {
            ...
            if (in_array($data['type'], ['trip', 'guideTrip', 'service', 'property'])) {

                $userPost = match ($data['type']) {
                    'service' => $reviewItem->provider,
                    'property' => $reviewItem->host,
                    default => $reviewItem->user,
                };
    ...
    public function deleteReview($data)
    {
        ...
        try {
            ...
            if ($reviewId) {
                $notifiableId = $reviewItem->provider_id ?? $reviewItem->user_id ?? $reviewItem->host_id;
            ...
            DB::rollBack();
            report($e);
            throw $e; // optionally rethrow or return error response
        }
    }
    ...
    public function reviewsLike($data)
    {
        ...
        try {
            ...
            $shouldSendFirebaseNotification = false;
            ...
            if ($existingLike) {
                ...
                $this->deleteReviewNotification($userReview->id, $review->id, $previousStatus);
                $shouldSendFirebaseNotification = true;
            } else {
            ...
            if (!$isSelfReview) {
                ...
                if ($shouldSendFirebaseNotification && !empty($tokens)) {
                    $notificationData = [
                        'notification' => [
