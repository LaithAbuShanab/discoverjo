<form wire:submit.prevent="submit">
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <div class="form-group">
                ...
                <input type="text" class="form-control" value="{{ $mainQuestion }}" readonly>
            </div>
        </div>
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select id="type" class="form-control" name="type" wire:change="onTypeChange" wire:model="type">
                    ...
                    <option value="category" {{ $type == 'category' ? 'selected' : '' }}>{{ __('app.category') }}</option>
                    <option value="subcategory" {{ $type == 'subcategory' ? 'selected' : '' }}>{{ __('app.subcategory') }}</option>
                    <option value="tag" {{ $type == 'tag' ? 'selected' : '' }}>{{ __('app.tag') }}</option>
                    <option value="feature" {{ $type == 'feature' ? 'selected' : '' }}>{{ __('app.features') }}</option>
                    <option value="cost" {{ $type == 'cost' ? 'selected' : '' }}>{{ __('app.cost') }}</option>
                    <option value="rating" {{ $type == 'rating' ? 'selected' : '' }}>{{ __('app.rating') }}</option>
    ...
    @foreach ($requiredAnswers as $index => $answer)
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <label class="form-label">{{ __('app.next-question') }}</label>
    ...
    @foreach ($additionalAnswers as $index => $answer)
        <div class="row mb-3">
            ...
            <div class="col-md-2" style="margin-top: 29.5px;">
